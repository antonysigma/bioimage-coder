...
struct Executor {
    ...
    static constexpr void dispatch(const repeat_for_t<Symbol, Integer, Callable>& command) {
     ...
     */
    template <typename T>
    static constexpr void dispatch(const T& command) {
        static_assert(sizeof...(ImageCaptureWorkers) == 4,
                      "Expecting 4 units of 24-to-1 camera multiplexer");
        ...
        if constexpr (std::is_same_v<Type, CloseAllCameraWorkers>) {
            ...
            if constexpr (has_completion_channel<Type>::value) {
                ...
                (ImageCaptureWorkers::capture_queue.push(new_capture_command), ...);
                ...
                (ImageCaptureWorkers::capture_queue.push(command), ...);
            }
    ...
    if constexpr (index < std::tuple_size_v<std::remove_reference_t<Protocol>>) {
        ...
        if constexpr (is_repeat_for_v<T>) {
            for (auto i = sub_protocol.range.begin; i < sub_protocol.range.end;
                ...
                std::apply(
                    [](auto&&... command) {
                        (Executor<MessageOverSerial, ImageCaptureWorkers...>::dispatch(command),
                         ...);
                    },
